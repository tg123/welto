	...
	function(self, event, name, ...)
		if event == "CHAT_MSG_SYSTEM" then
			...
			if newMemberName then
				local numTotalGuildMembers, numOnlineGuildMembers, _ = GetNumGuildMembers() -- 取公会在线人数
				print(numTotalGuildMembers) -- check
				for i = 1, numTotalGuildMembers do -- 取在线成员的信息
					local name, _, _, level, _, _, _, _, connected, _, class, _, _, _, _, _, guid = GetGuildRosterInfo(i)
					if not connected then return end

						print(numOnlineGuildMembers .. " " .. "OnLine") -- check
					if (newMemberName == name) then -- 比对新入会成员姓名和在线的已有成员姓名
						print(newMemberName) -- check
						C_Timer.After(3, function() Broadcast(name, level, class, guild) end)  -- Delay3秒后调用广播函数，实现广播
					end
				end
			end
